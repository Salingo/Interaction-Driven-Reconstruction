...
def work(id=0, cate='storage', file_name='46452_s00_p00_f00.pts', sample_per_part=SAMPLE_PER_PART, coe_friction=COE_FRICTION):
    ...
    save_path = f"{SAVE_DIR}/{cate}/{file_name}_{id}.pts"
    # load motion 
    ...
    for i in range(part_num):
        if np.linalg.norm(average_normal[i]) < 1e-6:
            for index in indices[i]:
                ...
                gen_data[i].append([p[0],p[1],p[2],v[0],v[1],v[2], i, 0])
            continue
        if i == refpart:
            for index in indices[i]:
                p = pcs[index][:3]
