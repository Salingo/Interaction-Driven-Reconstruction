...
class SegDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pc_start_xyz = np.concatenate([pc_start_xyz, np.zeros((pc_start_xyz.shape[0], 1))], axis=1).astype(np.float32)
        pc_end_xyz = pc_end_xyz.astype(np.float32)
        pc_end_xyz = np.concatenate([pc_end_xyz, np.ones((pc_end_xyz.shape[0], 1))], axis=1).astype(np.float32)
        ...
        pc_start_xyz = torch.from_numpy(pc_start_xyz)
        pc_end_xyz = torch.from_numpy(pc_end_xyz)
        ...
        return pc_start_xyz, pc_end_xyz, gt_start_seg, gt_end_seg, self.pid_list[index], self.pm_path_list[index], joint_info
