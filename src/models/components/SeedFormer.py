...
from src.utils.seed_former_utils import vTransformer, \
    PointNet_SA_Module_KNN, MLP_Res, MLP_CONV, fps_subsample, \
    query_knn, grouping_operation, get_nearest_index, indexing_neighbor
...
class SeedFormer(nn.Module):
    ...
    def __init__(self, feat_dim=512, embed_dim=128, num_p0=512, n_knn=20, radius=1, up_factors=None, seed_factor=2,
                 interpolate='three', attn_channel=True, use_joint=True):
        """
        Args:
        ...
        if use_joint:
            self.seed_generator = SeedGenerator(feat_dim=feat_dim+7, seed_dim=embed_dim, n_knn=n_knn, factor=seed_factor,
    ...
    def forward(self, partial_cloud, joint):
        """
        Args:
            partial_cloud: (B, N, 3)
            joint: (B, 7)
        ...
        if self.use_joint:
            feat = torch.cat([feat, joint.transpose(2,1)], dim=1)
...
def seedformer_dim128(**kwargs):
    model = SeedFormer(feat_dim=1024, embed_dim=128, n_knn=20, **kwargs)
    return model


def mini_seedformer_dim128(**kwargs):
    model = SeedFormer(feat_dim=64, embed_dim=32, n_knn=5, **kwargs)
    return model
...
if __name__ == '__main__':
    model = seedformer_dim128(up_factors=[1, 2, 2], num_p0=1024)
    model = model.cuda()

    x = torch.rand(8, 2048, 3)
    x = x.cuda()

    summary(model, input_size=[(8, 2048, 3),(8,1,7)])
    
    model_graph = draw_graph(model, input_size=[(8, 4096, 3), (8, 4096, 6)], device='cuda:0', expand_nested=True)
