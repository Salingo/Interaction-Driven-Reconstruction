...
class NetworkCritic(nn.Module):
    ...
    def critic_best_bak(self, pcs, pcs_dir):
        pcs_feat = self.pointNeXt(pcs).permute(0, 2, 1).contiguous()
    ...
    def critic_best(self, pcs, pcs_dir):
        B, N, _ = pcs.shape
        
        mean_score = torch.zeros(B, N, 1).to(pcs.device)
        best_d = torch.zeros(B, N, 3).to(pcs.device)
        best_score = torch.zeros(B, N, 1).to(pcs.device)
        for i in range(self.rd_cnt):
            rd = np.random.randn(B, N, 3)
            rd /= np.linalg.norm(rd, axis=2, keepdims=True)
            rd = torch.from_numpy(rd).to(pcs.device).float()
            pcs_rd = torch.cat([pcs, rd], dim=-1)
            pred_score = self.forward(pcs, pcs_rd)
            mean_score += pred_score
            mask = (best_score < pred_score).squeeze(-1)
            best_d[mask] = rd[mask]
            best_score[mask] = pred_score[mask]
        mean_score /= self.rd_cnt
